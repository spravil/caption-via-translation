...
def shift_tokens_right(input_ids: torch.Tensor, pad_token_id: int, decoder_start_token_id: int):
    ...
    # attention_mask = attention_mask.masked_fill_(shifted_input_ids == -100, 0)

    # replace possible -100 values in labels by `pad_token_id`
    shifted_input_ids.masked_fill_(shifted_input_ids == -100, pad_token_id)

    return shifted_input_ids  # , attention_mask
...
class FlorenceGemma2CrossAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
    ...
    def __init__(self, config: Gemma2Config, layer_idx: int):
        ...
        self.attn_logit_softcapping = self.config.attn_logit_softcapping
        self.sliding_window = False  # config.sliding_window if not bool(layer_idx % 2) else None
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        cross_input_shape = key_value_states.shape[:-1]
        cross_hidden_shape = (*cross_input_shape, -1, self.head_dim)
        key_states = self.k_proj(key_value_states).view(cross_hidden_shape).transpose(1, 2)
        value_states = self.v_proj(key_value_states).view(cross_hidden_shape).transpose(1, 2)
        ...
        # print(attention_mask.shape, query_states.shape, key_states.shape, value_states.shape)
        attn_output, attn_weights = attention_interface(
            self,
            query_states,
            key_states,
...
class FlorenceGemma2DecoderLayer(nn.Module):
    def __init__(self, config: Gemma2Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.config = config
        ...
        if layer_idx % config.encoder_attn_every_x_layers == 0:
            self.encoder_attn = FlorenceGemma2CrossAttention(config=config, layer_idx=layer_idx)
            self.encoder_attn_gate = nn.Parameter(torch.tensor([0.0]))
            self.post_encoder_attention_layernorm = Gemma2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        assert past_key_value is None, past_key_value  # TODO Fix
        ...
        if self.encoder_attn is not None:
            ...
            hidden_states = self.post_encoder_attention_layernorm(hidden_states)
            hidden_states = residual + hidden_states * self.encoder_attn_gate.tanh()
...
class FlorenceGemma2PreTrainedModel(PreTrainedModel):
    config_class = Gemma2Config
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["FlorenceGemma2DecoderLayer"]
    _skip_keys_device_placement = ["past_key_values"]
    _supports_flash_attn_2 = True
    _supports_sdpa = True
    _supports_flex_attn = True
...
class FlorenceGemma2Model(FlorenceGemma2PreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`Gemma2DecoderLayer`]
    ...
    def __init__(self, config: Gemma2Config):
        ...
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList(
            [FlorenceGemma2DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = Gemma2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = Gemma2RotaryEmbedding(config=config)
        self.gradient_checkpointing = False
...
class FlorenceGemma2LanguageModel(Florence2LanguagePreTrainedModel):
    _tied_weights_keys = []
    ...
    def __init__(self, config: Florence2LanguageConfig):
        ...
        self.decoder = FlorenceGemma2Model(config.gemma_config)
    ...
    def set_input_embeddings(self, value):
        raise Exception("set_input_embeddings should never be called")
        # self.shared = value
...
class FlorenceGemma2LanguageForConditionalGeneration(Florence2LanguagePreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]
    _tp_plan = {"lm_head": "colwise_rep"}

    def __init__(self, config: Florence2LanguageConfig):
        super().__init__(config)
        self.model = FlorenceGemma2LanguageModel(config)
        self.lm_head = nn.Linear(config.gemma_config.hidden_size, config.gemma_config.vocab_size, bias=False)
    ...
    def set_input_embeddings(self, value):
        self.get_decoder().embed_tokens = value
    ...
    def forward(
        ...
        if self.config.gemma_config.final_logit_softcapping is not None:
            lm_logits = lm_logits / self.config.gemma_config.final_logit_softcapping
...
class FlorenceGemma2PreTrainedModel(PreTrainedModel):
    config_class = FlorenceGemma2Config
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _skip_keys_device_placement = "past_key_values"
...
class FlorenceGemma2ForConditionalGeneration(FlorenceGemma2PreTrainedModel, GenerationMixin):
    accepts_loss_kwargs = False

    def __init__(self, config: FlorenceGemma2Config):
        super().__init__(config)
        ...
        language_model = FlorenceGemma2LanguageForConditionalGeneration(config=config.text_config)
    ...
    def forward(
        ...
        decoder_inputs_embeds: Optional[torch.FloatTensor] = None,
        labels: Optional[torch.LongTensor] = None,
        use_cache: Optional[bool] = False,  # TODO fix
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        **kwargs,
