...
class Florence2Config(PretrainedConfig):
    ...
    Args:
        ...
        projection_dim (`int`, *optional*, defaults to 1024):
            Dimension of the 1modal projection space.
...
class Florence2Decoder(Florence2LanguagePreTrainedModel):
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            # vocab_size = self.get_input_embeddings().num_embeddings  # Retrieve the vocab size from the embedding layer
            # # Check for out-of-bounds indices
            # invalid_mask = (input_ids < 0) | (input_ids >= vocab_size)
            # if invalid_mask.any():
            #     print("Decoder Warning: Found invalid input IDs.", input_ids[invalid_mask])
            #     input_ids[invalid_mask] = self.padding_idx  # Replace invalid indices
...
class Florence2LanguageModel(Florence2LanguagePreTrainedModel):
    ...
    def __init__(self, config: Florence2LanguageConfig):
        if config.use_encoder_tokenizer:
            self._tied_weights_keys = []
        super().__init__(config)

        padding_idx, vocab_size = config.pad_token_id, config.vocab_size

        if self.config.use_encoder_tokenizer:
            encoder_embedding = nn.Embedding(config.encoder_vocab_size, config.d_model, config.encoder_pad_token_id)
            self.encoder = Florence2Encoder(config, encoder_embedding)
            decoder_embedding = nn.Embedding(vocab_size, config.d_model, padding_idx)
            self.decoder = Florence2Decoder(config, decoder_embedding)
            self._tied_weights_keys = []
    ...
    def get_input_embeddings(self):
        if not self.config.use_encoder_tokenizer:
            return self.shared
        else:
...
class Florence2LanguageForConditionalGeneration(Florence2LanguagePreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if final_logit_softcapping is not None:
            lm_logits = lm_logits / final_logit_softcapping
...
class Florence2ForConditionalGeneration(Florence2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: Florence2Config):
        super().__init__(config)
        assert config.vision_config.model_type == "davit" or config.vision_config.model_type == "", (
    ...
    def _merge_input_ids_with_image_features(self, image_features, inputs_embeds, attention_mask=None):
        ...
        **kwargs,
        ...
        if inputs_embeds is None:
            ...
            if input_ids is not None:
                # vocab_size = (
                #     self.get_input_embeddings().num_embeddings
                # )  # Retrieve the vocab size from the embedding layer
                # # Check for out-of-bounds indices
                # invalid_mask = (input_ids < 0) | (input_ids >= vocab_size)
                # if invalid_mask.any():
                #     print("Warning: Found invalid input IDs.", input_ids[invalid_mask])
                #     input_ids[invalid_mask] = self.pad_token_id  # Replace invalid indices

                inputs_embeds = self.get_encoder().get_input_embeddings()(input_ids)
            # 2. Merge text and images
            if pixel_values is not None:
                # (batch_size, num_image_tokens, hidden_size)
                image_features = self._encode_image(pixel_values)
                inputs_embeds, attention_mask = self._merge_input_ids_with_image_features(
                    image_features, inputs_embeds, attention_mask=attention_mask
                )
